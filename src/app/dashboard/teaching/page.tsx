...
import { 
  BookOpen, LogOut, Plus, Trash2, FileText, ChevronRight, ChevronLeft, 
  BarChart3, Database, Clock, Code2, GraduationCap, Sparkles 
} from 'lucide-react';
      ...
      <header className="sticky top-0 z-30 w-full border-b border-zinc-200 bg-white/80 backdrop-blur-xl">
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <nav className="flex gap-1 -mb-px">
            ...
            <button
              className="flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 border-emerald-600 text-emerald-700 transition-colors"
            >
        ...
        <div className="flex flex-col gap-4 sm:gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            ...
            <Button 
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg transition-all hover:-translate-y-0.5"
            >
        ...
        <div className="space-y-4 sm:space-y-6">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 sm:py-24 bg-white rounded-xl border border-dashed border-zinc-200">
              <div className="w-14 h-14 sm:w-16 sm:h-16 bg-emerald-50 rounded-full flex items-center justify-center mb-3 sm:mb-4">
              ...
              <p className="text-zinc-500 mt-1 mb-4 sm:mb-6 text-sm text-center max-w-md">
                上传教材 PDF，AI 将自动分析章节结构，生成可编辑的课件 DSL
              </p>
              <Button onClick={() => setShowCreateForm(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white">
                创建第一个教研库
              </Button>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {currentKBs.map((kb, index) => (
                  <Card 
                    key={kb.id} 
                    className="group relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-emerald-200/30 sm:hover:-translate-y-1 border-zinc-200 bg-white cursor-pointer"
                    style={{ animationDelay: `${index * 50}ms` }}
                    onClick={() => router.push(`/dashboard/teaching/${kb.id}`)}
                  >
                    <CardHeader className="pb-3 sm:pb-4 pt-4 sm:pt-6 px-4 sm:px-6">
                      <div className="flex justify-between items-start mb-3 sm:mb-4">
                        <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg border border-emerald-100 bg-emerald-50 flex items-center justify-center text-emerald-600 group-hover:bg-emerald-100 transition-all duration-300 shadow-sm">
                      ...
                      <CardTitle className="text-base sm:text-lg font-semibold text-zinc-900 group-hover:text-emerald-700 transition-colors line-clamp-1">
                        {kb.name}
                      </CardTitle>
                    ...
                    <CardFooter className="pt-0 pb-4 sm:pb-5 px-4 sm:px-6">
                      <div className="w-full flex items-center text-sm font-medium text-zinc-400 group-hover:text-emerald-600 transition-colors gap-1 group-hover:gap-2 duration-300">
                        <span>生成课件</span>
            ...
            <CardContent className="px-4 sm:px-6 pb-6">
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <Button type="submit" className="flex-1 h-11 bg-emerald-600 hover:bg-emerald-700 text-white" disabled={creating}>
